## Geocode
##
## Borrowed from the \code{ggmap::geocode()} function from the \code{ggmap}
## package by David Kahle \email{david.kahle@@gmail.com} to remove dependencies
## of \code{ggmap} that are not necessary for \code{rcanvec}.
##
## modifications: added rjson:: prefix so that it is clear that fromJSON is
## from {rjson}, removed references to the %notin% operator, removed vectorization
## which requires dependence on dplyr.
##
## Not exported, just used in searchbbox()
##
...
##
## @param location a character scalar input (e.g.
